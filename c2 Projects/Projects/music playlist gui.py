...
def add_song():
    selected_song_index = song_library_listbox.curselection()  
    if selected_song_index:
        song_title = song_library_listbox.get(selected_song_index[0])
        playlist_listbox.insert(tk.END, song_title)
...
def remove_song():
    ...
    if selected_song_index:
        song_title = playlist_listbox.get(selected_song_index[0])
...
song_library_listbox = tk.Listbox()
song_library_scrollbar = tk.Scrollbar()
label = tk.Label(root, text="Song Library Listbox")
song_library_listbox.config(yscrollcommand=song_library_scrollbar.set)
song_library_scrollbar.config(command=song_library_listbox.yview)
...
#   1. Song deta   ils label (with the attribute assignment justify=tk.LEFT to
...
playlist_scroll = tk.Scrollbar()
playlist_label = tk.Label(root, text = "Playlist")
playlist_listbox.config(yscrollcommand=song_library_scrollbar.set)
playlist_scroll.config(command=song_library_listbox.yview)
...
song_library_scrollbar.grid(row=1, column=1, sticky='ns')
song_library_listbox.grid(row=1, column=0, sticky='nsew')
# Place the playlist heading, listbox, and scrollbar (9 points - 3 each):
playlist_label.grid(row=0, column=2, sticky='s')
playlist_scroll.grid(row=1, column=3, sticky='ns')
playlist_listbox.grid(row=1, column=2, sticky='nsew')
